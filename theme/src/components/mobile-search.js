...
function MobileSearch({onDismiss}) {
  const siteMetadata = useSiteMetadata()
  const {reset, results, isOpen, getComboboxProps, getInputProps, getItemProps, getMenuProps, highlightedIndex} =
    useSearch()
...
function MobileSearchWrapper() {
  ...
  return (
    <>
      ...
      <AnimatePresence>{isOpen ? <MobileSearch onDismiss={() => setIsOpen(false)} /> : null}</AnimatePresence>
    </>
  )
}
